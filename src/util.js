...
class FirebaseUtil {
    ...
    /* static createOptionsFromEnvironment() {
        let options = {};
        options = this.requiredOrFromEnvironment(options, 'credential', 'serviceAccount');
        options = this.requiredOrFromEnvironment(options, 'databaseURL', 'databaseURL');
        return options;
    } */
    ...
    static requiredOrFromEnvironment(options, key, env) {
        ...
        if (!options[key]) {
            throw `Firebase auth adapter requires an ${key}`;
        }
        return options
    }
